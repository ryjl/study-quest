package app.studyquest.tv.data.repo

import app.studyquest.tv.data.local.StoredUser
...
class CourseRepo @Inject constructor(
     ...
     */
    suspend fun fetchGradeTags(): List<app.studyquest.tv.data.remote.dto.GradeTagDto> =
        runCatching { api.fetchGradeTags() }
            .getOrElse { app.studyquest.tv.data.remote.dto.PresetGradeTags }

    /**
     * 学科目录。端点失败 → 空表(学科没有像 grade 那样的预设兜底,后端 DB-driven)。
     * 调用方用 [resolveSubject] 查询,空 catalog 时走 fallback(显示原始 key)。
     */
    suspend fun fetchSubjects(): List<app.studyquest.tv.data.remote.dto.SubjectDto> =
    ...
    suspend fun reportProgress(episodeId: Int, positionSeconds: Int, deltaWatchSeconds: Int) =
        api.reportProgress(
            app.studyquest.tv.data.remote.dto.ReportProgressRequest(
                episodeId = episodeId,
     ...
     */
    suspend fun fetchEpisodeSummary(episodeId: Int): app.studyquest.tv.data.remote.dto.EpisodeSummaryDto? {
        return try {
     ...
     */
    suspend fun fetchEpisodeAdvice(episodeId: Int): app.studyquest.tv.data.remote.dto.AdviceResponseDto {
        return try {
            api.fetchEpisodeAdvice(episodeId)
        } catch (e: HttpException) {
            if (e.code() == 404) {
                app.studyquest.tv.data.remote.dto.AdviceResponseDto(status = "unavailable")
            } else {
                throw e
            }
        }
