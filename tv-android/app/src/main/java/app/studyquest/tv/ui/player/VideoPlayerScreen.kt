...
import app.studyquest.tv.player.ProgressReporter
import app.studyquest.tv.player.ResumeWatchdog
...
private fun VideoPlayerContent(
    playInfo: app.studyquest.tv.data.remote.dto.PlayInfoDto,
    episodeId: Int,
    context: Context,
    baseUrl: String?,
    ...
    val exoPlayer = remember(playInfo.url) {
        ExoPlayer.Builder(context)
            ...
            val subtitleConfigs = playInfo.subtitles.map { sub ->
                val absoluteSubUrl = app.studyquest.tv.data.repo.UrlResolver.absolute(
                    baseUrl, sub.url,
                )
    ...
    var subtitleOptions by remember {
        mutableStateOf(
            app.studyquest.tv.domain.mergeSubtitleOptions(
                playInfo.subtitles, emptyList(),
            ),
        )
    }
    var audioOptions by remember { mutableStateOf(emptyList<app.studyquest.tv.domain.AudioOption>()) }
    // 当前选中的字幕索引(0 = 「无」/ 关闭)。
    ...
    LaunchedEffect(exoPlayer) {
        val tracksListener = object : Player.Listener {
            override fun onTracksChanged(tracks: androidx.media3.common.Tracks) {
                val nativeSubs = mutableListOf<app.studyquest.tv.domain.NativeSubtitleTrack>()
                val nativeAudios = mutableListOf<app.studyquest.tv.domain.NativeAudioTrack>()
                    ...
                    for (i in 0 until trackGroup.length) {
                        ...
                        when (type) {
                            androidx.media3.common.C.TRACK_TYPE_TEXT -> {
                                ...
                                nativeSubs.add(
                                    app.studyquest.tv.domain.NativeSubtitleTrack(
                                        id = id,
                                        title = format.label,
                            ...
                            androidx.media3.common.C.TRACK_TYPE_AUDIO -> {
                                nativeAudios.add(
                                    app.studyquest.tv.domain.NativeAudioTrack(
                                        id = id,
                                        title = format.label,
                                        language = format.language,
                                    ),
                ...
                val merged = app.studyquest.tv.domain.mergeSubtitleOptions(
                    playInfo.subtitles, nativeSubs,
                )
                subtitleOptions = merged
                // 音轨(契约第 2 节:过滤 no/auto 占位)。
                audioOptions = app.studyquest.tv.domain.audioOptions(nativeAudios)
                ...
                if (!subtitleAutoSelected && merged.size > 1) {
                    val defaultIdx = app.studyquest.tv.domain.defaultSubtitleIndex(merged)
                    if (defaultIdx > 0) {
    ...
    fun applySubtitle(index: Int) {
        ...
        when (opt.type) {
            app.studyquest.tv.domain.SubtitleType.OFF -> {
                params.setTrackTypeDisabled(androidx.media3.common.C.TRACK_TYPE_TEXT, true)
            }
            app.studyquest.tv.domain.SubtitleType.BACKEND -> {
            ...
            app.studyquest.tv.domain.SubtitleType.NATIVE -> {
                params.setTrackTypeDisabled(androidx.media3.common.C.TRACK_TYPE_TEXT, false)
    ...
    fun applyAudio(option: app.studyquest.tv.domain.AudioOption) {
    ...
    Box(
        ...
        val subtitleSizeDp = app.studyquest.tv.domain.subtitleSizeDp(subtitleSizeIndex)
                ...
                Column(
                    ...
                    Row(
                        ...
                        PlayerMenuButton(
                            ...
                            label = "字幕大小",
                            options = app.studyquest.tv.domain.SUBTITLE_SIZE_LABELS.mapIndexed { idx, label ->
