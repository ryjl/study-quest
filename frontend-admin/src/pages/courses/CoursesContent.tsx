...
export function CoursesContent({ onEdit, onChanged }: { onEdit: (c: Course) => void; onChanged: () => void }) {
  ...
  return (
    <div>
      ...
      {previewForCourse && (
        <PreviewPromptModal courseId={previewForCourse.id} courseTitle={previewForCourse.title} onClose={() => setPreviewForCourse(null)} />
      )}
    </div>
  );
}

// PreviewPromptModal 已抽到 components/ai/PreviewPromptModal.tsx,本文件和课程工作台
// Prompt tab 共用——让"编辑 prompt + 看效果"不依赖具体页面。
