...
//
// One row per (user, episode): a student has a SINGLE current quiz per lesson.
// "重做" (redo) answers the same set again (Answer rows accumulate, memory
// updates); "换题" (regenerate) deletes this row + its questions and re-runs the
// agent against the latest memory. The uniqueIndex enforces the single-row
// invariant so concurrent lazy-generation can't create duplicates.
type Quiz struct {
	ID            uint      `gorm:"primaryKey;autoIncrement"`
	EpisodeID     uint      `gorm:"uniqueIndex:idx_quiz_user_ep;not null"`
	UserID        uint      `gorm:"uniqueIndex:idx_quiz_user_ep;not null"`
	CourseID      uint      `gorm:"index;not null"`
