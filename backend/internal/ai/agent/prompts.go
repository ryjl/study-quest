...
【术语纠错硬规则——输出前必须执行】
字幕由 Whisper 机器转录,常有术语同音错字。典型例子:
- 象棋:车→居(错)、和棋→合棋(错)、马→码(错)、炮→泡(错)等;
- 数学:通分→同分(错)、约分→月分(错)、公分母→工分母(错)等。
理解时按正确含义读字幕;但**输出时一律写成正确术语**,不得保留同音错字。具体来说:
- headline / sections(标题与 points)/ key_points / methods / common_mistakes / concepts / takeaway 里出现的任何术语,都要纠正成规范写法(象棋的"车"不能写成"居","和棋"不能写成"合棋")。
- 如果课程提供了"术语字典"(在 user 消息的提示/QuizHint 里),**一律以字典为准**判定正误;字典里没有的,按学科常识纠正。
...
- 象棋:车(不能写"居")、和棋(不能写"合棋")、马(不能写"码")、炮(不能写"泡")。
- 数学:通分(不能写"同分")、约分(不能写"月分")、公分母(不能写"工分母")。
- 如果课程在 user 消息里提供了"术语字典"(QuizHint),**一律以字典为准**判定正误;字典里没有的,按学科常识纠正。
...
- 【Whisper 术语纠错】字幕是机器转录,常有术语同音错字(象棋:车→居、和棋→合棋、
  马→码;数学:通分→同分等)。你输出正文里涉及术语时,一律用规范写法,不保留字幕
  里的同音错字——象棋要说"车""和棋",不能写"居""合棋"。如果课程提供了 QuizHint
  术语字典,以字典为准判定;否则按学科常识纠正。
...
- 用真实的知识点名组织内容(从 episode summary 的 concepts/key_points 里来,例如"通分""公分母"
  ...
  这是后续异分母加减的基础"。
- 【Whisper 术语纠错】episode summary 是基于机器转录字幕生成的,可能残留术语同音错字
  (象棋:车→居、和棋→合棋、马→码;数学:通分→同分等)。你输出课程总览时,涉及术语
  一律用规范写法,不要照抄错字——象棋要说"车""和棋",不能写"居""合棋"。如果课程提供了
  QuizHint 术语字典,以字典为准判定;否则按学科常识纠正。
...
- 【Whisper 术语纠错】字幕是机器转录,常有术语同音错字(象棋:车→居、和棋→合棋、马→码;数学:通分→同分等)。你输出报告里涉及术语时,一律用规范写法,不保留字幕里的同音错字——象棋要说"车""和棋",不能写"居""合棋"。如果课程提供了 QuizHint 术语字典,以字典为准判定;否则按学科常识纠正。
