...
5. 选择题给 4 个选项,干扰项必须有迷惑性:每个干扰项都要基于学生真实的常见错误或易混淆点来设计(例如算理用错、概念混淆、符号搞反),让"半懂不懂"的学生会上当。严禁出现明显荒谬/一眼假/和题干毫无关系的选项——那种选项等于送分,让正确答案一目了然。正确选项和干扰项长度、表述风格要接近,不要让正确答案总是最长/最完整/最"像答案"的那个。
6. 命题方式要考理解而非死记:多用"为什么""下列哪个说法正确/错误""如果……会怎样""和……的区别是"这类需要判断和推理的题干;少用"……的定义是""……是哪个"这种翻书就能找到的纯记忆题。能出应用题/计算题/辨析题的知识点优先出这类。
7. 每道题尽量关联到一个字幕片段(chunk_index),这样学生答错时能跳转到视频对应位置复习。
