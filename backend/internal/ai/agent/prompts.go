...
- 【可选的富文本增强——鼓励,非必须】在 points / methods / common_mistakes / takeaway 这些字符串字段里,可以用 markdown 让内容更好读:多概念对比用 GFM 表格(| 列1 | 列2 | 语法),能让"两个概念的区别""步骤对照"一目了然;关键术语用 **加粗**;步骤/列举用 - 列表。当某个知识点用图比文字更清楚时(如"通分的三个步骤""几种分数的大小对比"),可在字段值里输出一个 SVG 图(把 SVG 源码放在 svg 围栏代码块里,即三反引号 + svg 开头的代码块)。SVG 只允许两种简单图型:简单流程图(方框+箭头,节点≤6、单行文字、无嵌套子图)或简单柱状图(≤5 柱、带数值标签);禁止时序图(sequenceDiagram)、ER 图、甘特图、饼图、思维导图、状态图、类图等一切复杂或渲染效果差的图型。SVG 要素:viewBox ≤ 800×400、只用 rect / line / polygon(做箭头)/ text 基础元素、配色用浅色填充(#EFF6FF / #F0FDF4 / #FEF2F2)+ 深色文字(#1E293B)+ 箭头 #64748B。没有合适的图就纯文字,不要硬凑——简单知识点用纯文字完全没问题,只在表格/图真能提升理解时才用。
