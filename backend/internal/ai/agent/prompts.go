...
- 【富文本增强——在合适场景下应当主动使用,不要默认纯文字】points / methods / common_mistakes / takeaway 这些字符串字段里,凡内容属于下列场景,**优先**用表格或 SVG 图而不是纯文字罗列:
  ...
  · 流程类(多步骤、有先后顺序、递进关系)→ SVG 简单流程图(方框+箭头)
  · 数据/分布类(几项数值的多少对比、占比)→ SVG 简单柱状图(≤5 柱、带数值标签)
  对纯定义、单点陈述、概念解释这类**不适合可视化**的内容,继续用纯文字,**不要硬凑**——没有合适的图就纯文字完全没问题。
  其它可用的 markdown:关键术语 **加粗**、步骤/列举用 - 列表。
  SVG 输出方式:把 SVG 源码放在 svg 围栏代码块里(三反引号 + svg 开头的代码块)。SVG 只允许两种简单图型:简单流程图(方框+箭头,节点≤6、单行文字、无嵌套子图)或简单柱状图(≤5 柱、带数值标签);禁止时序图(sequenceDiagram)、ER 图、甘特图、饼图、思维导图、状态图、类图等一切复杂或渲染效果差的图型。SVG 要素:viewBox ≤ 800×400、只用 rect / line / polygon(做箭头)/ text 基础元素、配色用浅色填充(#EFF6FF / #F0FDF4 / #FEF2F2)+ 深色文字(#1E293B)+ 箭头 #64748B。
...
- SVG **只允许简单流程图(方框+箭头,节点≤6)或简单柱状图(≤5 柱)**,禁止时序图 / ER 图 / 甘特图 / 饼图 / 思维导图 / 状态图 / 类图等复杂图型。SVG 要素:viewBox ≤ 800×400、只用 rect / line / polygon(做箭头)/ text 基础元素、配色用浅色填充(#EFF6FF / #F0FDF4 / #FEF2F2)+ 深色文字(#1E293B)+ 箭头 #64748B。
...
- 【可选的富文本增强——鼓励,非必须】可用 markdown 提升可读性:弱点对比用 GFM 表格(如"知识点 | 掌握度 | 建议")、知识点名用 **加粗**、复习建议用编号列表。当存在明确的流程性建议(如"先复习通分→再做异分母→最后综合")或数据对比时,可输出 SVG 图(SVG 源码放在 svg 围栏代码块里,即三反引号 + svg 开头的代码块)。SVG **只允许简单流程图(方框+箭头,节点≤6)或简单柱状图(≤5 柱)**,禁止时序图 / ER 图 / 甘特图 / 饼图 / 思维导图 / 状态图 / 类图等复杂图型。SVG 要素:viewBox ≤ 800×400、只用 rect / line / polygon(做箭头)/ text 基础元素、配色用浅色填充(#EFF6FF / #F0FDF4 / #FEF2F2)+ 深色文字(#1E293B)+ 箭头 #64748B。但 advice 的核心是诚恳的文字建议,图只是辅助,不要喧宾夺主——大部分建议用纯文字就够了。
...
- 【富文本增强——在合适场景下应当主动使用,不要默认纯文字】课程总览是给学生看的导览,纯文字大段
  ...
  · 学习路径/流程类(先学什么→再学什么→最后综合)→ SVG 简单流程图(方框+箭头)
  · 数据/分布类(各章节难度/重要性对比)→ SVG 简单柱状图(≤5 柱、带数值标签)
  对纯叙述、概念解释这类不适合可视化的内容,继续用纯文字,不要硬凑。
  其它可用 markdown:关键术语 **加粗**、章节列举用 - 列表。
  SVG 输出方式:把 SVG 源码放在 svg 围栏代码块里(三反引号 + svg 开头的代码块)。SVG **只允许
  简单流程图(方框+箭头,节点≤6)或简单柱状图(≤5 柱)**,禁止时序图 / ER 图 / 甘特图 / 饼图 /
  思维导图 / 状态图 / 类图等复杂图型。SVG 要素:viewBox ≤ 800×400、只用 rect / line /
  polygon(做箭头)/ text 基础元素、配色用浅色填充(#EFF6FF / #F0FDF4 / #FEF2F2)+ 深色文字
  (#1E293B)+ 箭头 #64748B。
