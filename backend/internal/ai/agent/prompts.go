...
- 【JSON 引号规则——违反会导致解析失败】所有字符串值(field 的内容)里如果要表达"引号"的含义,**绝对不要写裸 ASCII 双引号**(它会被当成字符串结束符,导致 JSON 在中途断裂、整个总结解析失败)。改用以下任一方式:
  · 中文引号「」或""(推荐,最自然)
  · 或 JSON 转义写成 反斜杠加双引号
  · 或用 **加粗** 强调(不需要引号语义时)
  正例:象棋级别「毕业」或 象棋级别**毕业**。反例(会炸):象棋级别"毕业"——这里的裸双引号让 JSON 断裂。
...
5. 【题干自足——最重要的一条】每道题的 stem 必须能脱离视频独立成立:学生隔几天回来、或学了别的课再回来,光看题干就能明白在问什么。
   ...
   - 正例(对):"本课在 16:30 左右讲到车九平四的四种走法时,红方走车九平四的主要目的是什么?"——题干自带背景,且带时间锚点,学生能定位到视频具体段落。
   - 【多盘棋/多例题的课——尤其关键】如果一节课复盘了多盘棋、或讲了多个例题(典型:象棋复盘课讲 3-5 盘棋、数学课讲了 3 道例题),题干必须写明指的是**哪一盘/哪一道**。search_subtitles 和 get_related_chunks 返回的字幕片段都带时间戳(如"(时间 1:00:46, 片段#50)"或"[出题时可引用时间锚点: 16:30]"),出题时**直接把这个时间点或盘次写进题干**。只说"本课讲到"是错的——学生隔几天回来分不清是第几盘。
     · 多盘棋正例:"本课复盘到约 37:16 那盘棋时,红方走车八平七吃马,黑方应如何应对?"——带时间锚点,即使复盘了 5 盘棋也能区分。
     · 多例题正例:"本课第 2 个例题(约 03:48)讲到的'顿挫'杀法,这步顿挫的关键作用是什么?"——带例题序号 + 时间,定位明确。
     · 反例(错):"本课讲到屏风马对中炮的开局时……"——一门复盘 5 盘棋的课,可能盘盘都讲屏风马,学生不知道是哪盘。
   - 纯概念题(如"什么是顿挫""通分的定义")不强制带时间锚点,这类题问的是知识点本身,带时间反而啰嗦。但凡是"指着视频某处/某盘棋/某个例题"的题(局面题、走法题、例题题、应用题),必须带时间或盘次锚点。
...
6. 题干自足性:任何含"这里/那个/上面/老师说的/如前所述/刚才/这步"等指代词、或脱离本课单独看就答不上的题,判 fail。题干必须让学生光看文字就明白问什么,不依赖视频画面或上下文。
   - 【多盘棋/多例题课的锚点检查】如果本课复盘了多盘棋、讲了多个例题(典型:象棋复盘课、数学多例题课),凡"指着某盘棋/某个例题"的题(局面题、走法题、例题题),题干必须带时间锚点(如"约 37:16 那盘棋")或盘次/例题序号(如"第 2 个例题")。只说"本课讲到"而无法区分是哪盘棋/哪道例题的,判 fail。
