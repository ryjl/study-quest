...
type CourseService interface {
	...
	//
	// whisperHint/quizHint 是 admin 表单的两个 AI 提示 textarea,service 层组装
	// 成 model.AIConfig 序列化进 Course.AIConfigJSON 单列(前向兼容:加新配置项
	// 不必改 schema)。老 AIHint 列不再写,旧数据走 Effective* 回退。详见
	// model.Course.AIConfigJSON / AIConfig / SetAIConfig。
	CreateCourse(title string, grades []model.Grade, subjectID uint, contentType model.ContentType, coverURL string, tagIDs []uint, attachmentJSON string, whisperHint, quizHint string, aiSummaryEnabled, aiQuizEnabled bool) (*model.Course, error)
	UpdateCourse(id uint, title string, grades []model.Grade, subjectID uint, contentType model.ContentType, coverURL string, tagIDs []uint, attachmentJSON string, whisperHint, quizHint string, aiSummaryEnabled, aiQuizEnabled bool) (*model.Course, error)
...
func (s *courseService) CreateCourse(title string, grades []model.Grade, subjectID uint, contentType model.ContentType, coverURL string, tagIDs []uint, attachmentJSON string, whisperHint, quizHint string, aiSummaryEnabled, aiQuizEnabled bool) (*model.Course, error) {
	...
	c.SetAIConfig(model.AIConfig{WhisperHint: whisperHint, QuizHint: quizHint})
...
func (s *courseService) UpdateCourse(id uint, title string, grades []model.Grade, subjectID uint, contentType model.ContentType, coverURL string, tagIDs []uint, attachmentJSON string, whisperHint, quizHint string, aiSummaryEnabled, aiQuizEnabled bool) (*model.Course, error) {
	for _, g := range grades {
		if !g.Valid() {
			return nil, errors.New("invalid course grade value: " + string(g))
		}
	...
	// AI 配置走单一 JSON 列(AIConfigJSON)。service 层把 admin 表单的两个
	// textarea(whisperHint/quizHint)组装成 AIConfig 再序列化。加新配置项时
	// 只需扩 AIConfig struct + 表单,不用改 DB schema。老 AIHint 列清空,让
	// Effective* 不再回退到旧 blob(避免陈旧提示残留)。
	c.SetAIConfig(model.AIConfig{WhisperHint: whisperHint, QuizHint: quizHint})
	c.AIHint = ""
